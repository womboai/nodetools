...
class MemoProcessor:
    ...
    async def parse_group(
        ...
        """
        Parse a complete group of memos.
        ...
        if structure.is_standardized_format:
            ...
            return await LegacyMemoProcessor.parse_group(
                group,
                credential_manager=credential_manager,
                message_encryption=message_encryption,
                node_config=node_config
    ...
    async def construct_group(
        ...
        """
        if response_params.legacy_memo:
            return await LegacyMemoProcessor.construct_group(
                response_params,
                credential_manager=credential_manager,
                message_encryption=message_encryption,
                node_config=node_config
...
def chunk_memos(
    ...
    Args:
        ...
        max_size: Maximum size in bytes for each complete Memo object
        legacy_memo: Whether the memo is a legacy memo
    ...
    for chunk_number in range(1, num_chunks + 1):
        ...
        if legacy_memo:
            chunk_memo_data = f"chunk_{chunk_number}__{chunk.decode('utf-8', errors='ignore')}"
        else:
...
class StandardizedMemoProcessor:
    ...
    async def construct_group(
        ...
        """
        await asyncio.sleep(0)  # Ensure this is a coroutine
        processed_data = response_params.memo_data
        ...
        base_memo = Memo(
            ...
            memo_type=response_params.memo_type,
